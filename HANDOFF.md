...
Against the kamui capture (lower is closer): untouched track **3.169**, current render **1.808**, codec floor **1.14**.

**Cross-checked against an independent reimplementation** ([`Rosemoe/sdvx-sfx-renderer`](https://github.com/Rosemoe/sdvx-sfx-renderer), IDA-based). It independently confirms the peak-filter table and curves, 11 of 13 effect ids, Wobble's column layout and waveforms, the laser mirroring, AUTO TAB's 2-indexing, and the `chain` combination model. It supplied the Wobble fix (§1.6). Full comparison, remaining conflicts and head-to-head scores: spec §9. Head-to-head over 12 charts: ours ahead 9/12 on effect rendering alone, 11/12 with the full mix.

### 1.2 Rejected by measurement — do not revisit

Parallel/additive effect combination (`--laser-mode add`, 0 wins of 16); scaling mix depth (1.0 optimal); the device EQ *after* the SE mix (2.029 vs 1.924); additive SE layering (1.889 vs 1.858); freezing the duck between lasers (`--duck-hold`, −0.34); Tape Stop Ex with envelope floor 0.0 (+1.92 vs +3.22).

*Removed from this list*: "an int16 round-trip between stages" stood here as rejected, but `stage()` performs exactly that and `STAGE_CLIP` defaults to on, with spec §8.1 arguing the engine does clip so the default should keep it. Code and doc directly contradicted each other. Whatever the original experiment rejected, the claim as written was wrong; `--no-stage-clip` still exists to A/B it if anyone wants to re-measure.
...
### 1.6 Wobble — the long-standing defect, fixed

For most of the project Wobble was the one effect measurably failing: it moved the audio more than any other DSP, and moved it *away* from the recording (~+0.02 exclusive gain against +1.3 to +4.1 for everything else). The DSP was right all along; the unit conversion was not.

**C6 is a rate in cycles per beat, not a period in beats** — the wrapper takes its reciprocal (`periodSec = (60/BPM) / C6`). We multiplied instead of dividing, running the ubiquitous `4.00` row 16x too slow: one sweep per 4 beats instead of 4 wobbles per beat. Spec §4.9, §5.4.

Measured over every capture-matched chart that uses Wobble: **13/13 improved, none worse**, mean exclusive gain **+0.077 → +0.955** (delta +0.878 dB, 3247 frames). `--wobble-legacy-period` restores the old reading.

Found by comparing against an independent reimplementation (§4), then confirmed in our own disassembly before adopting.

### 1.7 `#TAB PARAM ASSIGN INFO` sweep — implemented, direction resolved and confirmed

The other real win from comparing against an independent reimplementation, and the second thing adopted from it after Wobble (§1.6). `#TRACK AUTO TAB` (§6.3) was already applying the borrowed FX-button effect at its *authored* parameters; the missing half — a C4 = 6 laser sweeping one of that effect's parameters between two bounds, refreshed every 512 samples — is now implemented and on by default (`--no-param-assign-sweep` to disable).

The direction that had been unresolved (which end of the laser maps to which of C2/C3) came from their code: `value = min + (max - min) · clip(laserValue, 0, 1)`, `param1`/`param2` selected by chain position. Confirmed by measurement, not just adopted on their say-so: scored on the 148 charts where an AUTO TAB span actually overlaps an active C4 = 6 laser segment, **+0.698 dB mean over 16 sampled charts**, 5 of 8 non-trivial charts improved (the other 8 of 16 saw no assignment active on the measured span and scored identically). Full numbers: `specs/audio_engine.md` §9.5.

Also settled by the same reimplementation and confirmed independently: **`C4 = 6` is not "no effect"** — it is the laser's way of saying "I am the control source for this parameter sweep," not "do nothing." The inherited format notes' "no effect (?)" was right about the laser's own filter output and wrong about the laser being inert.

---

## 2. Unresolved

None of these block conversion work, and none is a known *defect* — the last of those (Wobble) closed with §1.6. What remains is refinement and untraced detail.
...
### 2.3 Effect state continuity — done for Wobble, open for BitCrusher and Gate
...
### 2.5 Laser + FX combination: the binary and the measurement still disagree
...
### 2.6 No measured floor for the non-kamui recordings

kamui has a codec floor of 1.14, which is what makes "1.808" interpretable. The recordings in `scripts/shared/reference/ksh/` have no such floor measured, so "+1.07 gain" has no yardstick — nobody knows whether 0.2 or 2.0 of headroom remains. Cheapest estimate: score a chart's *idle* frames, where the render is the dry track by construction.

### 2.7 Camera — spin length needs a ground-up re-derivation
...
### 2.8 Tape Stop Ex's envelope floor — still unexplained; the three-phase reading didn't fix it

`specs/audio_engine.md` §9.5. The independent reimplementation's three-phase reading of id 10 (attack → hold → release, vs this project's preroll → spin-up) was tested as the leading hypothesis for why §1.5's envelope floor won't fit — and it didn't resolve it. A/B'd on 16 charts: mean delta −0.108, frame-weighted −0.082, 8 of 16 charts prefer it and 6 prefer ours, with individual swings up to ±3–5 dB. Genuinely inconclusive, not a rejection with a clear margin like Gate or BitCrusher got.

So the floor is back to being a real open question rather than a "probably the three-phase model" one. `--tapestop-ex-3phase` is kept for anyone who wants to dig further — the struct-offset evidence that both sides are reading the same routine still stands — but it is not the default and the mystery is unsolved.
