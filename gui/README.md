...
| `argspec.py` | Introspects `apply_chart.build_arg_parser()` into the Advanced panel's controls, skipping whatever the worker already computes per chart (`-d`, `-a`, `-o`, `--se-bank-dir`, `--dry`). |
