...
| `convert_worker.py` | The background job queue: per (song, difficulty), calls `notes/convert.py`'s `convert()` (with real metadata via its `meta=` param) then `apply_chart.py`'s `main()`, in-process, on one worker thread. Streams stdout into the Debug console. The `po=`/`plength=` half of that metadata is measured rather than read (`audio/preview.py`) and cached per song, since the preview window belongs to the track and not to the difficulty. |
| `argspec.py` | Introspects `apply_chart.build_arg_parser()` into the Advanced panel's controls, skipping whatever the worker already computes per chart (`-d`, `-a`, `-o`, `--se-bank-dir`, `--dry`). The three chart-side checkboxes at the top of that panel - **Standard slam gap**, **Remove pretilt** and **Preview offset (po/plength)** - are not `apply_chart.py` flags (the first two are `notes/convert.py` arguments, the third selects whether the worker measures the preview window at all), so `app.py` places them by hand above the introspected list. So is the **KSH version** dropdown, which sits in the toolbar instead: it decides which editors and players can read the output at all (a v2 file's laser curves are invisible to KSM v1.xx), which is a choice to make before converting rather than a tuning knob to leave alone. |
