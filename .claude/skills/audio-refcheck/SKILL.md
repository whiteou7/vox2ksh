...
* **`masscheck.py` scores the dampened peak EQ**, because it invokes `apply_chart.py` without `--peak-gain-scale`/`--peak-max-gain` and the CLI defaults are deliberately tamed (`specs/audio_engine.md` §7.1). That is fine for before/after comparison as long as both runs match. To reproduce the spec's transcribed-model numbers, pass `--extra="--peak-gain-scale 1.0 --peak-max-gain 15"` to **both** runs.
