...
| [`../scripts/camera/pretilt.py`](../scripts/camera/pretilt.py) | Measures the tick gaps between literal `tilt=0` lines and the laser run they precede, and between the following `tilt=normal` and that run's start - the pretilt-cancellation idiom (see below). |
...
- **Tilt intervention style varies wildly by song.** Counting `tilt=` events per laser run across the 30 reference charts: 5 songs use **zero** manual tilt lines at all (pure `tilt=normal` throughout - `aim_higher`, `aqua_luna_rium`, `chakra`, `komorebi_ni_saku`, `shiawase_usagi_peko_miko_marin`), while others range from light touch-ups (~0.1-0.6 events/run) to dense hand-animation (up to 7.37 events/run in `furiko_doll/mxm.ksh` - a near-continuous manual reproduction of the tilt curve, not a simple bracket idiom). Per direction, this is a low-priority piece; noted here because it explains why the pretilt-pattern search below only converges cleanly on a subset of songs.
...
## Tilt: automatic baseline + pretilt cancellation + manual passthrough
...
**Pretilt cancellation - confirmed real, pattern found, implemented.** Direction: KSM's own auto-tilt anticipates an upcoming laser and starts tilting before the arcade would; charters insert `tilt=0` shortly before a laser run to cancel that anticipation, then `tilt=normal` once the run actually starts to hand back control. `pretilt.py`'s survey confirms this:

- Restricting to "fresh section starts" (a laser run beginning after ≥1 beat of complete silence on both lanes, `pretilt.py`'s `section_start_runs`) rather than every run - the pretilt idiom is plausibly inserted once per silence -> laser transition, not per individual run within an ongoing passage.
- The cleanest example, `foolish_again/exh.ksh`, has **6/6** occurrences of the exact same shape: `tilt=0` exactly 24 cells (1/8 measure) before the run starts, `tilt=normal` exactly at the run's start tick.
- Across the full "sparse-intervention" subset (songs with 0.05-0.7 tilt-events per laser run, i.e. targeted fixes rather than continuous hand-animation), the modal pre-gap (tilt=0 to run start) is 0 cells (41/97), and the modal post-gap (the following tilt=normal to run start) is also 0 cells (11/38, the single largest bucket in an otherwise spread distribution) - both point at the same idiom, with real variance in exactly how many cells of lead time different charters used (0, 24, 48, 96, 168, 192... all occur).
- **Implemented** in `camera.py` as `compute_tilt_events`: `tilt=normal` baseline; at every fresh-section laser-run start, `tilt=0` is inserted `PRETILT_LEAD=24` cells early (the one fully-consistent example's value - a judgment call among the observed spread, flagged as such) and `tilt=normal` right at the run start. Manual `Tilt` vox segments (when present) override this and are passed through as literal floats, since they're genuine charter-authored camera work, not auto-tilt.
- Not resolved: the actual triggering condition KSM uses (how far ahead it anticipates, whether it depends on laser speed/BPM). `PRETILT_LEAD`/`PRETILT_SILENCE_MIN` are tunable constants in `camera.py` if this needs revisiting.
...
4. **`PRETILT_LEAD`'s exact value (24)** was fit from one example (`foolish_again/exh.ksh`) but now has a second, independent exact match on `2226_gryphone_etia_5m` (59/59 tilt events matched exactly, pretilt brackets included) - stronger than one data point, but the observed spread elsewhere (0-192+ cells across the full reference set) means it's still not proven universal; the real trigger may depend on something not yet identified (laser speed into the run, BPM, or similar) that happens to resolve to 24 in both fully-clean examples found so far.
5. Per direction, DLL work (chart reader `FUN_180239810`, the unexamined gameplay-event kinds in `FUN_180407200`) stays the fallback once the above are worth revisiting, not the starting point.
